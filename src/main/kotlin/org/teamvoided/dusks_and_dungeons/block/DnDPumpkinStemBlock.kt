...
class DnDPumpkinStemBlock(private val gourdBlock: Block, settings: Settings) : CropBlock(settings) {
    ...
    override fun randomTick(state: BlockState, world: ServerWorld, pos: BlockPos, random: RandomGenerator) {
        ...
        if (random.nextInt((25.0f / moisture).toInt() + 1) == 0) {
            ...
            if (age < maxAge) world.setBlockState(pos, state.with(AGE, age + 1), NOTIFY_LISTENERS)
            else {
                ...
                if (canGrowOnBlock(world.getBlockState(growPos.down()), world, growPos)) {
                    world.setBlockState(growPos, gourdBlock.defaultState, NOTIFY_LISTENERS)
                    world.setBlockState(pos, state.with(FACING, direction), NOTIFY_LISTENERS)
                }
            }
        }
    }
    ...
    override fun fertilize(world: ServerWorld, random: RandomGenerator, pos: BlockPos, state: BlockState) {
        ...
        world.setBlockState(pos, blockState, NOTIFY_LISTENERS)
    ...
    companion object {
        ...
        fun shape(height: Double): VoxelShape = createCuboidShape(7.0, 0.0, 7.0, 9.0, height, 9.0)
    }
}